...
within guests of the ServerOS hypervisor. These guests may be either
ServerOS Zones or KVM virtual machines.
...
generally shipped with the OS (in the case of ServerOS) or in the package (e.g.
...
this repository are clients that communicate using this protocol.  The ServerOS
hypervisor provides a [common set][datadict] of supported
base metadata keys for guests to consume, as well as the ability to support
arbitrary additional user-provided metadata.

In a ServerOS container/zone guest, a UNIX domain socket is used to communicate
...
The tools currently build and function on ServerOS and various Linux
